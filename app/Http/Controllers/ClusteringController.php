...
class ClusteringController extends Controller
{
    public function detail($id){
        $dok = Dok::where('idDok',1)->first();
    }
    public function cekHasil(){
        $dok = Dok::select('hasilPre','totKata')->where('idDok',1)->first();
        $dat = explode(' ',$dok->hasilPre);
        $daftarKata = array_count_values($dat);
        $kata       = array_keys($daftarKata);
        $tf = array();
        foreach ($kata as $t) {
            $tf[$t]['a'] = $daftarKata[$t]/$dok->totKata;
            $tf[$t]['b'] = $daftarKata[$t]/count($dat);
        }
        echo "dat = ";
        print_r($dat);echo "<br> daftarkata = ";
        print_r($daftarKata);echo "<br>kata = ";
        print_r($kata);echo "<br> tf = ";
        print_r($tf);
    }
    ...
    public function kel(Request $request)
    {
        ...
        foreach ($clusterList as $key => $value) {
            $clustera[] = Dok::select('judul')->whereIn('idDok', $value)->get();
        }
        foreach ($clustera as $key => $value) {
            echo "<br><br>";
            foreach ($value as $t) {
                echo "<br>- ".strtolower($t->judul);
            }
            // var_dump($value);
        }
